...
import firrtl.transforms.{ConstantPropagation, DeadCodeElimination}
...
case class MultiClockModule(target: ModuleTarget, domains: Seq[ClockDomain]) extends SingleTargetAnnotation[ModuleTarget] {
  override def duplicate(n: ModuleTarget) = copy(target = n)
}
...
private object ModuleDomainScanner {
  ...
  private def filterClocks(module: String, annos: Seq[ClockAnnotation]): Seq[LocalClock] = {
    ...
    val subPorts = annos.filter(a => a.target.component.isEmpty && a.target.path.nonEmpty)
    ...
    val byLocalName = local.groupBy(_.target.ref) ++ subPorts.groupBy(a => a.target.path.last._1.value + "." + a.target.ref)
    val clocks = byLocalName.toSeq.map { case (name, as) =>
