...
object AssertSignalPass extends Transform with DependencyAPIMigration {
  ...
  private def onModule(m: ir.DefModule, portNames: Map[String, String]): ir.DefModule = m match {
    case mod: ir.Module =>
      ...
      val anyFailed: ir.Expression = if (asserts.isEmpty) { Utils.False() }
      else {
      ...
      mod.copy(ports = m.ports :+ outPort, body = ir.Block(stmts.toList))
    case other => other
  }

  private def onStmt(
    s:         ir.Statement,
    asserts:   mutable.ListBuffer[ir.DefNode],
