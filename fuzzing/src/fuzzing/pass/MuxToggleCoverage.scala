...
import fuzzing.MuxToggleOpAnnotation
...
object MuxToggleCoverage extends Transform with DependencyAPIMigration {
  ...
  private def coverToggle(ctx: ModuleCtx, conds: List[ir.Expression]): (List[ir.Statement], List[Annotation]) = {
    ...
    val prevResetName = ctx.namespace.newName("prev_reset")
    val prevResetRef = ir.Reference(prevResetName, Utils.BoolType, RegKind)
    val prevReset = ir.DefRegister(ir.NoInfo, prevResetName, Utils.BoolType, ctx.clock, Utils.zero, prevResetRef)
    ...
    val stmts: List[ir.Statement] = conds.flatMap { muxCond =>
      ...
      val prevCondRef = ir.Reference(ctx.namespace.newName(name + "_prev"), Utils.BoolType, RegKind)
      val prevCond = ir.DefRegister(
        ir.NoInfo,
        prevCondRef.name,
        Utils.BoolType,
        ctx.clock,
      ...
      val toggle = ir.DefNode(
        ir.NoInfo,
        ctx.namespace.newName(name + "_toggle"),
        ir.DoPrim(PrimOps.Xor, Seq(condRef, prevCondRef), Seq.empty, Utils.BoolType)
      )
      // Tracks toggles but ignores them when reset is on
      val toggleNoReset = ir.DefNode(
        ir.NoInfo,
        ctx.namespace.newName(name + "_toggleNoReset"),
        Utils.and(ir.Reference(toggle), Utils.not(Utils.or(ctx.reset, prevResetRef)))
      )

      if (!ctx.useFullToggle) {

        val toggleNoResetCov = ir.Verification(
          ir.Formal.Cover,
          ir.NoInfo,
          ctx.clock,
          ir.Reference(toggleNoReset),
          Utils.one,
          ir.StringLit(""),
          ctx.namespace.newName(name + "_toggleNoResetCov")
        )
        ...
        val toggleStoreRef = ir.Reference(ctx.namespace.newName(name + "_toggleStore"), Utils.BoolType, RegKind)
        val toggleStore = ir.DefRegister(
          ir.NoInfo,
          toggleStoreRef.name,
          Utils.BoolType,
          ctx.clock,
        ...
        val fullToggle = ir.DefNode(
          ir.NoInfo,
          ctx.namespace.newName(name + "_fullToggleNoReset"),
          Utils.and(toggleStoreRef, toggleNoResetRef)
        )
        val fullToggleCov = ir.Verification(
          ir.Formal.Cover,
          ir.NoInfo,
          ctx.clock,
          ir.Reference(fullToggle),
          Utils.one,
          ir.StringLit(""),
          ctx.namespace.newName(name + "_fullToggleNoResetCov")
        )
  ...
  private def coverPseudoToggle(ctx: ModuleCtx, conds: List[ir.Expression]): (List[ir.Statement], List[Annotation]) = {
    val stmts = conds.flatMap { cond =>
      ...
      val oneCover = ir.Verification(
        ...
        Utils.not(ctx.reset),
        ir.StringLit(""),
        ctx.namespace.newName(name + "_one")
      )
      val zeroCover = ir.Verification(
        ir.Formal.Cover,
        ir.NoInfo,
        ctx.clock,
        Utils.not(nodeRef),
        Utils.not(ctx.reset),
        ir.StringLit(""),
        ctx.namespace.newName(name + "_zero")
      )
